...
export class ReviewRunExecutor {
      ...
      for (const { runId, agent } of jobs) {
        await this.repo
          .completeAgentRun(runId, {
            ...
            tokensIn: 0,
            tokensOut: 0,
            // Nothing ran, so cost is unknown rather than zero.
            costUsd: null,
            findingsCount: 0,
  ...
  private async runOneAgent(
    ...
    try {
      ...
      // ---- Persist review + findings + markReviewed + agent_runs ------------
      // ONE transaction (ReviewRepository.finalizeRun): a crash mid-persist can
      // never leave a review without its run completion. completeAgentRun's
      // status='running' guard is preserved inside — a run cancelled mid-flight
      // keeps its terminal 'cancelled' status.
      const { review, findings: findingRows } = await this.repo.finalizeRun({
        review: {
          workspaceId,
          prId: pull.id,
          agentId: agent.id,
      ...
      await this.repo
        .completeAgentRun(runId, {
          ...
          tokensIn: 0,
          tokensOut: 0,
          // A cancelled run may have burned tokens, but reviewPullRequest throws
          // through checkCancelled and the accumulated counters die with it —
          // so the honest value here is unknown, not zero.
          costUsd: null,
          findingsCount: 0,
          grounding: '0/0 passed',
          error: msg,
